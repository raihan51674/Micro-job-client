...
const DashboardLayout = ({ children, userRole = 'worker' }) => { // Default role for demo
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);

  // Define navigation links based on roles (conceptual)
  const navLinks = {
    worker: [
      { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard },
      { name: 'Available Tasks', href: '/dashboard/tasks', icon: ClipboardList },
      { name: 'My Submissions', href: '/dashboard/submissions', icon: ClipboardList },
      { name: 'Withdraw Earnings', href: '/dashboard/withdraw', icon: DollarSign },
      { name: 'Settings', href: '/dashboard/settings', icon: Settings },
    ],
    buyer: [
      { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard },
      { name: 'Post New Task', href: '/dashboard/post-task', icon: ClipboardList },
      { name: 'Manage Tasks', href: '/dashboard/my-tasks', icon: Briefcase },
      { name: 'Purchase Coins', href: '/dashboard/purchase-coins', icon: DollarSign },
      { name: 'Settings', href: '/dashboard/settings', icon: Settings },
    ],
    admin: [
      { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard },
      { name: 'Manage Users', href: '/dashboard/users', icon: Users },
      { name: 'Manage Tasks', href: '/dashboard/admin-tasks', icon: ClipboardList },
      { name: 'Withdraw Requests', href: '/dashboard/withdraw-requests', icon: DollarSign },
    ...
    >
      ...
      >
                ...
                <li key={index}>
                  <Link href={link.href}>
                    <motion.div
                      ...
                      whileTap={{ scale: 0.98 }}
                      className="flex items-center p-3 rounded-lg text-gray-300 hover:text-blue-400 transition-colors duration-200 cursor-pointer group"
                    >
                      <Icon className="h-5 w-5 mr-3 text-purple-400 group-hover:text-blue-400 transition-colors duration-200" />
        ...
        <div className="mt-auto pt-6 border-t border-white/10 text-center">
          ...
          <button className="mt-3 px-4 py-2 text-sm bg-red-600 hover:bg-red-700 rounded-lg transition-colors">
            Logout
          </button>
        </div>
      </aside>
        ...
        <AnimatePresence>
          {isSidebarOpen && (
            <motion.div
              ...
              className="fixed inset-0 bg-black/70 z-50 md:hidden" // Dark overlay
